...
t=1.0
beta=100.0
U_list = [0.0,0.4,0.7,1.0,1.4,1.8,2.2,2.6,3.0,3.4,3.8,4.2,4.6,5.0,5.2,5.3,5.4,5.5,5.6,5.7,5.8,5.9,6.0,6.1,6.2,6.3,6.4,6.5,6.6,6.7,6.8,6.9]
symbol_list = ['-o','-^','-*','-s','-8','-h','-H','-s','-o','-o','-^','-s',"-o"]
data = open("all_observable_%s_re"%beta,'w')
print >> data, " # U_i(1)   density(2)   double_occu(3)  kinetic_energy(4) interaction_enegy(5) total_energy(6)"
for i,U in enumerate(U_list):
	d = loadtxt("observables_U%s_beta%s_re"%(U,beta))
	#print d
	print >> data, d[0], d[1], d[2], d[3], d[4], d[5]
data.close()

data = loadtxt("all_observable_%s_re"%beta)
...
plt.xlabel("U/t")
plt.savefig("energy_U_HM_beta%s_re.eps"%beta,format="eps")
plt.show()
