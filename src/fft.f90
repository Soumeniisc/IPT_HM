...
contains
  ...
  subroutine fft_t2w(parm_,y,z)
	...
	!if (j==1 .or. j== parm_%N_tau) then
	!	w(j)=0.5*y(j)*exp(-xj*dble(parm_%N_tau-1)*dble(j-1+0.5)*pi/dble(parm_%N_tau))
	!else 
	w(j)=y(j)*exp(-xj*dble(parm_%N_tau-1)*dble(j-1+0.5)*pi/dble(parm_%N_tau))
	!end if
    end do
    !print*, "calling soumens fft"
    call dfftw_plan_dft_1d(plan,parm_%N_tau,w,z,FFTW_BACKWARD,FFTW_ESTIMATE)
    call dfftw_execute(plan)
    call dfftw_destroy_plan(plan)
    
    do k=1, parm_%N_tau
       z(k)=z(k)* exp( xj*dble(2*(k-1))*pi/dble(2*parm_%N_tau) )*parm_%beta/dble(parm_%N_tau)
    end do
    deallocate(w)
  ...
  subroutine fft_w2t(parm_,z,y)
    ...
    complex(kind(0d0)), allocatable :: w(:),m(:)
    integer                         :: plan(8), j, k
    
    include 'fftw3.f'
    allocate(w(parm_%N_tau))
    allocate(m(parm_%N_tau))
    do j=1, parm_%N_tau
       w(j)= z(j)*exp( -xj*dble(2*j - parm_%N_tau-1)*pi/dble(2.0*parm_%N_tau) ) ! this thing only added
    end do
   
    call dfftw_plan_dft_1d(plan,parm_%N_tau,w,m,FFTW_FORWARD,FFTW_ESTIMATE)
    call dfftw_execute(plan)
    call dfftw_destroy_plan(plan)
    
    do j=1, parm_%N_tau
       y(j)=real(dble(m(j)*exp( xj*dble(parm_%N_tau-1)*dble(j-1)*pi/dble(parm_%N_tau)))/parm_%beta)
    end do
